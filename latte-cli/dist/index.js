...
program
    ...
    try {
        ...
        output.error(`Initialization failed: ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
    }
});
// latte add <pkg>
program
    ...
    try {
        ...
        if (paymentConfig) {
            const paymentSuccess = await paymentHandler.handlePayment(paymentConfig, options.user || "anonymous");
        ...
        output.error(`Failed to add package: ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
    }
});
// latte install
program
    ...
    try {
        ...
        output.error(`Installation failed: ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
    }
});
// latte remove <pkg>
program
    ...
    try {
        ...
        output.error(`Failed to remove package: ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
    }
});
// latte list
program
    ...
    try {
        ...
        output.error(`Failed to list packages: ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
    }
});
// latte set-price
program
    ...
    try {
        ...
        output.error(`Failed to set price: ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
    }
});
