...
export class PaymentHandler {
    ...
    constructor(apiBase: string = process.env.LATTE_API_BASE || "http://localhost:4000") {
        this.apiBase = apiBase;
    }

    async fetchPaymentRules(packageName: string): Promise<PaymentConfig | null> {
        try {
    ...
    private async handleRequiredPayment(
        config: PaymentConfig,
        userId: string
    ): Promise<boolean> {
        output.paymentInfo({
        ...
        const confirmed = await this.waitForPaymentConfirmation(
            config.packageName,
            userId,
            60
        );
    ...
    private async handleOptionalDonation(
        config: PaymentConfig,
        userId: string
    ): Promise<boolean> {
    ...
    private generatePaymentUri(config: PaymentConfig): string {
    ...
    async setPackagePrice(
        ...
        try {
            ...
            throw new Error(
                `Failed to set package price: ${error instanceof Error ? error.message : String(error)
                }`
            );
        }
    }
