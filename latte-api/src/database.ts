...
export class DatabaseManager {
    ...
    constructor(dbPath: string = "./latte.db") {
    ...
    // Package operations
    upsertPackageRule(rule: PackageRule): PackageRule {
    ...
    getPackageRule(name: string): PackageRule | null {
        const stmt = this.db.prepare(`
    ...
    getAllPackageRules(): PackageRule[] {
        const stmt = this.db.prepare(`
      SELECT id, name, price, required, wallet_address as walletAddress,
             chain, token_symbol as tokenSymbol
      FROM packages
    ...
    markPaymentConfirmed(packageName: string, userId: string, txHash?: string): boolean {
        const stmt = this.db.prepare(`
      UPDATE payments
      SET confirmed = 1
    ...
    getPaymentStatus(packageName: string, userId: string): boolean {
        const stmt = this.db.prepare(`
      SELECT confirmed
      FROM payments
    ...
    `);
        ...
        const row = stmt.get(packageName, userId) as any;
        return row !== undefined;
    }

    getPaymentsByPackage(packageName: string): Payment[] {
        const stmt = this.db.prepare(`
    ...
    `);
        ...
        const rows = stmt.all(packageName) as any[];
        return rows.map((row) => ({
            id: row.id,
    ...
    getTotalDonations(packageName: string): number {
        const stmt = this.db.prepare(`
    ...
    `);
        ...
        const row = stmt.get(packageName) as any;
        return row.total;
    }
